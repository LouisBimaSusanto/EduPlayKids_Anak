...
export default function Game({ game, onComplete, onClose }) {
    ...
    if (option === roundData.target) {
      ...
      setTimeout(() => {
        if (currentRound < 4) {
          ...
          onComplete({ completed: true, score: 100 });
        }
    ...
    <div className="w-full h-full flex flex-col items-center justify-between p-8 relative overflow-hidden">
      ...
      <AnimatePresence>
        {gameState === "success" && (
          ...
          >
             {/* Central Burst */}
             <div className="absolute top-1/4 left-1/4 text-7xl md:text-[15rem] animate-[ping_1s_ease-out_forwards]">🎆</div>
             <div className="absolute top-1/3 right-1/4 text-7xl md:text-[12rem] animate-[ping_1.2s_ease-out_forwards] animation-delay-200">🎇</div>
             <div className="absolute bottom-1/3 left-1/3 text-7xl md:text-[18rem] animate-[ping_1.5s_ease-out_forwards] animation-delay-500">🎆</div>
